const JSONExtend = {
   ...
   * @return {JSON}
   */
  sort: (json, callback = undefined) => {
    const sortedJSON = {};

    for (const key of Object.keys(json).sort(callback)) {
      sortedJSON[key] = json[key];
    }

    return sortedJSON;
  },
  /** Initialize array elements as key
   * @param {array} array element as key
   * @param {any} value
   *
   * @example
   * JSONExtend(['a', 'b'], 0)
